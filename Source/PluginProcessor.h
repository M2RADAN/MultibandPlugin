...
public:
    ...
    static constexpr int fftOrder = 10; // 2048
    static constexpr int fftSize = 1 << fftOrder;

    // Методы доступа для анализатора
    std::atomic<bool>& getIsFftDataReady() { return isFftDataReady; } // Используем этот флаг
    const std::array<float, fftSize / 2>& getFftMagnitudes() const { return fftMagnitudes; } // Отдаем магнитуды
    // =============================
