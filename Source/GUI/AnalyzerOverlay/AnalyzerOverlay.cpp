...
namespace MBRP_GUI
{
    ...
    float AnalyzerOverlay::xToFrequency(float x, const juce::Rectangle<float>& graphBounds) const
    {
        ...
        freq = juce::jlimit(lowMidXoverParam.getNormalisableRange().start, maxFreq, freq); // Нижняя граница Low/Mid
        freq = juce::jlimit(minFreq, midHighXoverParam.getNormalisableRange().end, freq);   // Верхняя граница Mid/High
