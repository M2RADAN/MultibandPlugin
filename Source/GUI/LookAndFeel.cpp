...
{
    // ПРОСТОЙ ПРИМЕР: Рисуем стандартный слайдер, но можно кастомизировать
    // LookAndFeel_V4::drawLinearSlider (g, x, y, width, height, sliderPos, minSliderPos, maxSliderPos, style, slider);

    // Или простой кастомный вид:
    using namespace juce;
    auto bounds = Rectangle<int>(x, y, width, height).toFloat();
    // Рисуем трек
    ...
    auto thumbPos = sliderPos; // sliderPos уже в пикселях
    auto thumbRect = Rectangle<float>(thumbWidth, bounds.getHeight())
        .withCentre(Point<float>(thumbPos, bounds.getCentreY()));
    g.setColour(slider.findColour(Slider::thumbColourId)); // Используем цвет из setupSlider
